...
| RISK-003 | Anomaly thresholds (Phase 05) and forecasting method (Phase 06) were designed from small-sample industry research (`RESEARCH.md` R-001/R-002) and the official 7-row example, not yet validated against real seeded volume. | Medium — a threshold that floods or empties the Action Queue undermines the whole differentiation strategy. | IN_PROGRESS | **Idle/anomaly-threshold half RESOLVED**, twice over: calibrated against Phase 02's seeded data (17 historical rows, 10 flagged/7 clean, no boundary ambiguity — `DECISIONS.md`'s "RISK-003 calibration result"), then implemented and independently re-verified in Phase 05's real `GET /api/anomalies` output, producing the exact predicted split (`phases/PHASE-05-anomaly-detection.md`). **Forecasting-method half still OPEN** — Phase 06 task 06.1 must still pick moving-average vs. exponential smoothing against this same seeded data (tracked as `Q-002`). |
